...
anno_root = '/media/icml/My_Passport/datasets/COCO2017/annotations'
...
def trans_anno(ori_file, target_file, is_val):
    ...
    if not isfile(ori_anno):
        no_ori = True
    if not file_exist and not no_ori:
    ...
    if no_ori:
        print(
            '''WARNING! There is no annotation file find at {}. 
            Make sure you have put annotation files into the right folder.'''.format(ori_anno)
        )
